...
- **ATTRIBUTED** — credited to Thompson by others; no primary utterance located.
...
| Peter Seibel, *Coders at Work*, ch. 12 | 2009 |
...
- the caller-enumeration / never-guard-a-single-call-site procedure and
  ...
  still non-firing and was reverted; v1.3 tests a third instruction class —
  tool-literal — see benchmarks/results/.)
...
## Commonly misattributed — not used as Thompson

- "Write programs that do one thing and do it well" — **Doug McIlroy** (BSTJ 57(6), 1978).
- The "5 Rules of Programming" — **Rob Pike** ("Notes on Programming in C", 1989).
- "Debugging is twice as hard as writing the code..." — **Brian Kernighan** (with Plauger, 1978).
- "You are not expected to understand this" — **Dennis Ritchie**, by his own account.
- "Keep it simple, stupid" — **Kelly Johnson**, Lockheed Skunk Works.
- "The most effective debugging tool is still careful thought, coupled with judiciously placed print statements." — **Kernighan** ("Unix for Beginners", 1979).
