...
// each file shows the same task answered with no skill vs with ken, same model.
...
const pick = (re, label) =>
  j.results.results.find((r) => isHaiku(r.provider.id) && r.prompt.label === label && re.test(r.vars.task));

const rows = [];
for (const [re, slug, title] of meta) {
  const b = pick(re, 'baseline (no skill)'), p = pick(re, 'ken');
...
## Without Ken — ${bL} lines of code

${b.response.output.trim()}

## With Ken — ${pL} lines of code
