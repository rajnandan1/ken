...
function correctnessTimeoutMs() {
  const value = Number.parseInt(process.env.KEN_CORRECTNESS_TIMEOUT_MS || '', 10);
...
function tmpFile(ext, content) {
  const p = path.join(os.tmpdir(), `ken-bench-${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`);
