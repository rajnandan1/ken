# 2026-08-26: Maintenance-over-time v1, sonnet round replicates the null
...
The sonnet results match the haiku null across the measured rates.
...
- **r6 root-cause: ken 0/3 again (0/6 across both models; baseline 3/6).**
  ...
  a ruleset interaction. Ken's minimality pressure (brute
...
- **Rewrite-on-rot: 1 rewrite in 24 rot cells across both rounds.** The
...
- **r9 accent-insensitive search: 6/6 failed again** (12/12 across models).
  Every session hand-rolls `.lower()` matching. The probe expectation ("the
  way users expect search to work across
...
method. The root-cause probe points against ken in both model rounds.
The next measured step is ruleset iteration: make the rewrite and root-cause
rules operational, re-run, and keep changes that move these numbers. Record
each iteration here.
