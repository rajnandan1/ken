# 2026-08-26 — Iteration 3 verdict: REVERT. The instruction avenue is closed.
...
caller that comes back" — the same concreteness that made the countable
rewrite trigger fire.
...
ruleset remains v1.1.0, and the instruction avenue for this rule is formally
closed.**

## The finding, now on rock
...
Meanwhile the same aspirational→procedural treatment fixed the rewrite rule
on its first try (0/2 → 2/2, holding across three rounds at median, with a
1-run wobble in each of the last two — the trigger is strong, ~5/6, not
deterministic).

Conclusion, stated as a limit of the product: **on haiku single-shot
sessions, no injected instruction — however concrete — induces cross-file
...
wasn't going to do. Remaining options are structural (enforced
