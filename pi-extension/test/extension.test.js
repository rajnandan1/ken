...
import kenExtension from "../index.js";
...
function withTempConfig(fn) {
  const tempConfigHome = mkdtempSync(join(tmpdir(), "ken-test-"));
  const previousXdg = process.env.XDG_CONFIG_HOME;
  const previousHide = process.env.KEN_HIDE_STATUS;
  process.env.XDG_CONFIG_HOME = tempConfigHome;
  delete process.env.KEN_HIDE_STATUS;
  ...
  return Promise.resolve()
    ...
    .finally(() => {
      ...
      if (previousHide === undefined) delete process.env.KEN_HIDE_STATUS;
      else process.env.KEN_HIDE_STATUS = previousHide;
      rmSync(tempConfigHome, { recursive: true, force: true });
    });
}

test("extension registers Ken commands", () => {
  const { commands } = createPiHarness();

  assert.deepEqual([...commands.keys()].sort(), ["ken", "ken-audit", "ken-debt", "ken-gain", "ken-help", "ken-review"]);
});

test("/ken updates session mode and injects instructions", async () => withTempConfig(async () => {
  const { commands, events, appendedEntries } = createPiHarness();
  const ctx = createCommandContext();

  await events.get("session_start")({ reason: "startup" }, ctx);
  await commands.get("ken").handler("ultra", ctx);
...
test("before_agent_start guards missing event and missing systemPrompt", async () => withTempConfig(async () => {
  ...
  // A null/undefined event must not crash, and still injects the ruleset.
  for (const bad of [undefined, null]) {
    const r = await events.get("before_agent_start")(bad, ctx);
    assert.ok(r.systemPrompt.includes("KEN MODE ACTIVE"));
    assert.ok(!r.systemPrompt.includes("undefined"), "must not contain the literal 'undefined'");
  }

  // An event without a systemPrompt must not prepend the literal "undefined".
  const empty = await events.get("before_agent_start")({}, ctx);
  assert.ok(empty.systemPrompt.includes("KEN MODE ACTIVE"));
  ...
  assert.ok(withBase.systemPrompt.includes("KEN MODE ACTIVE"));
}));
...
test("skill alias commands delegate to Pi skill commands", async () => {
  ...
  await commands.get("ken-review").handler("", ctx);
  await commands.get("ken-audit").handler("", ctx);
  await commands.get("ken-debt").handler("", ctx);
  await commands.get("ken-gain").handler("", ctx);
  await commands.get("ken-help").handler("", ctx);
...
test("stop ken disables persistent instructions", async () => withTempConfig(async () => {
  const { commands, events } = createPiHarness();
  const ctx = createCommandContext();

  await events.get("session_start")({ reason: "startup" }, ctx);
  await commands.get("ken").handler("ultra", ctx);
  await events.get("input")({ text: "stop ken", source: "interactive" }, ctx);
...
test("a request mentioning normal mode stays active", async () => withTempConfig(async () => {
  ...
  const result = await events.get("before_agent_start")({ systemPrompt: "BASE" }, ctx);
  assert.match(result.systemPrompt, /KEN MODE ACTIVE/);
}));
...
test("status bar renders the mode and flips active on agent_start", async () => withTempConfig(async () => {
  ...
  const statusWrites = [];
  const ctx = createCommandContext({
    sessionManager: { getEntries: () => [{ type: "custom", customType: "ken-mode", data: { mode: "ultra" } }] },
  ...
  assert.equal(statusWrites.at(-2).key, "ken");
...
test("KEN_HIDE_STATUS hides the indicator but keeps ken active", async () => withTempConfig(async () => {
  process.env.KEN_HIDE_STATUS = "1";
  const { events } = createPiHarness();
  const statusWrites = [];
  const ctx = createCommandContext({
    sessionManager: { getEntries: () => [{ type: "custom", customType: "ken-mode", data: { mode: "ultra" } }] },
  ...
  assert.match(injected.systemPrompt, /KEN MODE ACTIVE/, "ruleset must still inject while status is hidden");
}));

test("config.hideStatus hides the indicator but keeps ken active", async () => withTempConfig(async () => {
  mkdirSync(join(process.env.XDG_CONFIG_HOME, "ken"), { recursive: true });
  writeFileSync(join(process.env.XDG_CONFIG_HOME, "ken", "config.json"), JSON.stringify({ hideStatus: true }));
  const { events } = createPiHarness();
  const statusWrites = [];
  const ctx = createCommandContext({
    ui: { notify() {}, setStatus: (key, text) => statusWrites.push({ key, text }), theme: { fg: (_c, t) => t } },
...
test("KEN_HIDE_STATUS=0 does not hide the indicator", async () => withTempConfig(async () => {
  process.env.KEN_HIDE_STATUS = "0";
  const { events } = createPiHarness();
  const statusWrites = [];
  const ctx = createCommandContext({
    sessionManager: { getEntries: () => [{ type: "custom", customType: "ken-mode", data: { mode: "ultra" } }] },
