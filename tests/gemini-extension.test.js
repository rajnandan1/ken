...
// always-on context, commands/*.toml for /ken + /ken-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring ken.
...
const REUSED_COMMANDS = ['commands/ken.toml', 'commands/ken-review.toml'];
const REUSED_SKILLS = ['skills/ken/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. Ken's
...
test('manifest names the ken extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the ken rules', () => {
  const manifest = loadManifest();
