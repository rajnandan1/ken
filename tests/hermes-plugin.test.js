...
const commands = ['ken', 'ken-review', 'ken-audit', 'ken-debt', 'ken-gain', 'ken-help'];
const skillCommands = commands.filter((name) => name !== 'ken');

const root = path.join(__dirname, '..');

// ken: probe once; on Windows `python3` is the Store-alias stub that fails
...
test('Hermes plugin manifest matches runtime skills, hooks, commands, and package version', () => {
  ...
  assert.match(manifest, /^name:\s*ken$/m);
...
test('Hermes plugin registers every shipped skill under the ken namespace', () => {
  const output = python(String.raw`
import importlib.util, json, pathlib
spec = importlib.util.spec_from_file_location('ken_hermes_plugin', '__init__.py')
...
`);
  ...
  assert.ok(data.commands.includes('ken'));
  assert.ok(data.commands.includes('ken-review'));
});

test('Hermes plugin builds mode-aware injected context from the canonical skill', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ken-config-'));
  const output = python(String.raw`
...
`, { XDG_CONFIG_HOME: tmp });
  ...
  assert.match(ctx, /KEN MODE ACTIVE — level: ultra/);
  assert.match(ctx, /rather rewrite a thing than argue with it/);
...
test('Hermes mode config respects env, config file, off, and invalid command behavior', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ken-config-'));
  fs.mkdirSync(path.join(tmp, 'ken'), { recursive: true });
  fs.writeFileSync(path.join(tmp, 'ken', 'config.json'), JSON.stringify({ defaultMode: 'lite' }));
  const output = python(String.raw`
import importlib.util, json
spec = importlib.util.spec_from_file_location('ken_hermes_plugin', '__init__.py')
mod = importlib.util.module_from_spec(spec)
...
ctx = Ctx()
mod.register(ctx)
status_before = ctx.commands['ken']('')
invalid = ctx.commands['ken']('maximum')
status_after = ctx.commands['ken']('')
print(json.dumps({
...
`, { XDG_CONFIG_HOME: tmp, KEN_DEFAULT_MODE: 'ultra' });
  const data = JSON.parse(output);
  assert.match(data.default, /level: ultra/);
  assert.equal(data.off, '');
  assert.match(data.status_before, /Ken mode: ultra/);
  assert.match(data.invalid, /Usage:/);
  assert.match(data.status_after, /Ken mode: ultra/);
});
...
`);
  const { ctx } = JSON.parse(output);
  assert.match(ctx, /KEN MODE ACTIVE — level: review/);
  assert.match(ctx, /Review diffs for Thompson-mode violations/);
  assert.match(ctx, /net: -<N> lines, -<M> layers, trusted base -<K> deps possible/);
  assert.doesNotMatch(ctx, /^---/);
});

test('Hermes /ken command changes mode and pre_llm_call injects current context', () => {
  const output = python(String.raw`
import importlib.util, json
spec = importlib.util.spec_from_file_location('ken_hermes_plugin', '__init__.py')
mod = importlib.util.module_from_spec(spec)
...
ctx = Ctx()
mod.register(ctx)
message = ctx.commands['ken']('ultra')
...
`);
  ...
  assert.match(data.context, /KEN MODE ACTIVE — level: ultra/);
});
...
cases = {}
for text in ['/ken-review x', '/ken_audit repo', '/ken-debt', '/ken-help', '/status', 'hello']:
    cases[text] = mod.rewrite_gateway_command(event=Event(text))
print(json.dumps(cases, sort_keys=True))
`);
  const data = JSON.parse(output);
  assert.match(data['/ken-review x'].text, /ken-review/);
  assert.match(data['/ken_audit repo'].text, /ken-audit/);
  assert.match(data['/ken_audit repo'].text, /repo/);
  assert.match(data['/ken-debt'].text, /ken-debt/);
  assert.match(data['/ken-help'].text, /ken-help/);
