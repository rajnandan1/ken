...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'ken-uninstall-'));
...
const flagPath = path.join(claudeDir, '.ken-active');
fs.writeFileSync(flagPath, 'full');

const configDir = path.join(temp, 'config-home', 'ken');
...
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash /some/path/ken-statusline.sh' },
}));
...
assert.equal(
  ...
  undefined,
  'ken statusLine entry must be removed',
);
...
// #374: a combined statusline (another plugin && ken) must keep the other
// plugin's part — uninstall must not nuke the whole command or leave a husk.
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash ~/caveman-statusline.sh && bash /p/ken-statusline.sh' },
}));
...
assert.equal(
  ...
  'a combined statusLine must keep the non-ken command',
);
...
const malformedSettings = '{ "statusLine": { "command": "ken-statusline.sh", broken';
