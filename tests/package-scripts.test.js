...
test('root npm test covers bundled subprojects', () => {
  ...
  assert.match(packageJson.scripts.test, /npm test --prefix ken-mcp/);
});

test('CI installs MCP dependencies before root npm test', () => {
  const workflow = fs.readFileSync(path.join(root, '.github', 'workflows', 'test.yml'), 'utf8');

  assert.match(workflow, /npm install --prefix ken-mcp/);
  assert.ok(
    workflow.indexOf('npm install --prefix ken-mcp') < workflow.indexOf('npm test'),
