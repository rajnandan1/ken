...
Ken's second loop step is _steal, don't invent_. Check the platform first.

Before reaching for a package, scan these common cases. Your app already ships
with the platform, and platform maintainers own its updates.
...
```js
// ken: 3 lines beats a dependency
let t;
const debounce =
    (fn, ms) =>
    (...args) => {
...
| `uuid` (v4)                   | `crypto.randomUUID()`                                   |
| `ms` (parse duration strings) | keep `ms`; it is useful and tiny                        |
...
Wrapper dependencies often follow this sequence:
...
Install the library when the native solution lacks required browser support,
misses an edge case, or causes friction at scale.
