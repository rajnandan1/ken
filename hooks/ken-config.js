#!/usr/bin/env node
// ken — shared configuration resolver
//
// Resolution order for default mode:
//   1. KEN_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/ken/config.json (any platform, if set)
//      - ~/.config/ken/config.json (macOS / Linux fallback)
//      - %APPDATA%\ken\config.json (Windows fallback)
//   3. 'full'
...
// Only embed the plugin install path in a statusline shell command when it's
// made of ordinary path characters. An allowlist beats escaping every shell's
// metacharacters; a hostile clone path (quotes, &, $, backtick, ;, etc.) falls
// back to manual setup instead. Allows : \ / for normal Windows and POSIX paths.
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'ken');
  }
  ...
  return path.join(os.homedir(), '.config', 'ken');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.KEN_DEFAULT_MODE;
  // A default must be a runtime level (off/lite/full/ultra); review is a
  // session-only mode, never a valid default. Validate against RUNTIME_MODES
  // so a stray env var or config can't make review the default.
...
// Silence the pi "Ken loaded" startup toast while keeping ken active.
// KEN_QUIET_STARTUP=1 (or any truthy value; 0/false/empty mean "show it")
// takes precedence, else config.quietStartup === true. Mirrors getHideStatus.
function getQuietStartup() {
  const env = process.env.KEN_QUIET_STARTUP;
  if (env !== undefined) {
...
// Hide the status-bar indicator while keeping ken active.
// KEN_HIDE_STATUS=1 (or any truthy value; 0/false/empty mean "don't hide")
// takes precedence, else config.hideStatus === true.
function getHideStatus() {
  const env = process.env.KEN_HIDE_STATUS;
  if (env !== undefined) {
    const v = env.trim().toLowerCase();
    return v !== '' && v !== '0' && v !== 'false' && v !== 'no';
  }
