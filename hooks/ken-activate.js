#!/usr/bin/env node
// ken — Claude Code SessionStart activation hook
//
...
const { getDefaultMode, getClaudeDir, isShellSafe } = require('./ken-config');
...
if (!isCodex && !isCopilot) try {
  ...
  const nudgeFlagPath = path.join(claudeDir, '.ken-statusline-nudged');
  if (!hasStatusline && !fs.existsSync(nudgeFlagPath)) {
    try { fs.writeFileSync(nudgeFlagPath, ''); } catch (e) { /* best-effort */ }
    const isWindows = process.platform === 'win32';
    const scriptName = isWindows ? 'ken-statusline.ps1' : 'ken-statusline.sh';
    ...
    if (isShellSafe(scriptPath)) {
      ...
      // ken: install path has shell metacharacters — don't embed it in a
