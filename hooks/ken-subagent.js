#!/usr/bin/env node
// ken — Claude Code SubagentStart hook
//
...
// "^general$" is exact. Unset means inject into every subagent.

const { getKenInstructions } = require('./ken-instructions');
const { readMode, writeHookOutput } = require('./ken-runtime');

const mode = readMode();

// Absent flag or off → ken isn't active; inject nothing.
if (!mode || mode === 'off') {
  process.exit(0);
}
...
function inject() {
  try {
    writeHookOutput('SubagentStart', mode, getKenInstructions(mode));
  } catch (e) {
...
let matcherRe = null;
try {
  if (process.env.KEN_SUBAGENT_MATCHER) {
    matcherRe = new RegExp(process.env.KEN_SUBAGENT_MATCHER, 'i');
  }
} catch (e) {
  matcherRe = null;
}
...
// fires; the default path must not wait on stdin or it would stall every
// subagent spawn.
if (!matcherRe) {
  inject();
  process.exit(0);
